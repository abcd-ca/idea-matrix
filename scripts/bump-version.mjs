...
//
// Commit the result on a branch. When it reaches main, the Release workflow
// sees a version with no release yet, tags it and publishes the release.
...
console.log(`${current} -> ${next} in ${FILES.length} files and package-lock.json`);
...
function bump(version, how) {
  ...
  switch (how) {
    ...
    default: return /^\d+\.\d+\.\d+$/.test(how ?? "") ? how : null;
  }
}
