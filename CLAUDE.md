...
- `apps/web/src/lib/store.ts` is the Zustand store; `apps/web/src/lib/file-session.ts` owns the file handle, autosave and the external-change watcher (which merges a change from elsewhere into unsaved edits, three-way through core's `mergeDocuments`); `apps/web/src/lib/storage/local-file.ts` wraps the File System Access API, `storage/google-drive.ts` the Drive API, `storage/target.ts` puts both behind one `SaveTarget`, and `storage/same-device.ts` is the BroadcastChannel that tells other tabs on the same device about a save.
- `apps/web/src/lib/site.ts` is what search engines see: the site URL, each page's title, description and whether it is indexed (`pageMetadata` is every page's `metadata` export), the structured data in the root layout, and the list `app/sitemap.ts` and `app/robots.ts` build from. English only, since a static export has one `<head>` per page.
- `apps/web/src/lib/preferences.ts` and `preferences-store.ts` are the device preferences (language and theme): one validated record in local storage under `ideamatrix.preferences`, a second Zustand store separate from the document, and the one inline script the root layout runs before first paint. Nothing about them goes in the matrix file. `apps/web/src/lib/i18n.ts` initializes react-i18next from the JSON under `apps/web/src/locales/<language>/<namespace>.json`; unit tests in `apps/web/test` (vitest).
...
- End-to-end tests: `npm run test:e2e` runs the Playwright suite in `apps/web/e2e` (Chromium only, in CI after the build; two projects, the desktop specs under `e2e/` and the phone-layout specs under `e2e/phone/` on a phone viewport with touch emulation) against the static export, so run `npm run build` first when `apps/web/out` is missing or stale. The server listens on port 3100; set `E2E_PORT` to run a second worktree's suite alongside, since Playwright reuses whatever is already on the port and would test the other worktree's export. The tests stand in for the file dialogs by replacing `window.showSaveFilePicker` and `window.showOpenFilePicker` with functions that return a handle from the browser's origin private file system, a real `FileSystemFileHandle`, so the app's file code runs unchanged and a test reads the file back through a second handle. Google Drive has a stand-in of the same kind in `e2e/drive-stand-in.ts`: Google's two scripts and the Drive endpoints are answered from the test process, so two browser contexts can work one fake file as two devices would. `@playwright/test` is pinned to 1.62 because the Chromium that ships with 1.63 crashes when such a handle comes back out of IndexedDB after a reload; try a later release when one exists.
